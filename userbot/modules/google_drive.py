...
async def generate_credentials(gdrive):
    ...
    if helper.get_credentials(str(gdrive.sender_id)) is not None:
        await gdrive.edit("Token has already been authorized.")
    ...
    msg = await gdrive.respond(
        "Go to your Botlogs group to authenticate token."
    )
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
        url_msg = await conv.send_message(
            "Please go to this URL:\n"
            f"{auth_url}\n,authorize then reply the code to this message."
        )
        r = conv.wait_event(
...
async def create_app(gdrive):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            await gdrive.edit("Credentials are empty, please generate them.")
            return False
